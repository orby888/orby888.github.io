...
(function () {
  ...
  /* ---------- preloader (load-gated, <=1.2s, once per session) ---------- */
  var pre = $('#pre');
  if (pre) {
    var seen = false;
    try { seen = sessionStorage.getItem('gs-pre') === '1'; } catch (e) {}
    if (reduced || seen) {
      // cache/session skip: no curtain at all on repeat visits or reduced-motion
      ...
      var reveal = function () {
        ...
        setTimeout(function () { pre.classList.add('gone'); document.body.classList.add('go'); }, 600);
      };
      // reveal as soon as the page is ready, hard-capped at 600ms -> total <=1.2s
      if (document.readyState === 'complete') { reveal(); }
      else { addEventListener('load', reveal); setTimeout(reveal, 600); }
    }
  } else {
    document.body.classList.add('go');
  }
